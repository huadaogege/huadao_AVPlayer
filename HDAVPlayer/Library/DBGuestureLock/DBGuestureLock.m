...
+(instancetype)lockOnView:(UIView*)view onPasswordSet:(void (^)(DBGuestureLock *lock, NSString *password))onPasswordSet isUnLockType:(BOOL)isUnLockType {
    
    return [DBGuestureLock lockOnView:view offsetFromBottom:60.f onPasswordSet:onPasswordSet isUnLockType:isUnLockType];
}

+(instancetype)lockOnView:(UIView*)view offsetFromBottom:(CGFloat)offset onPasswordSet:(void (^)(DBGuestureLock *lock, NSString *password))onPasswordSet isUnLockType:(BOOL)isUnLockType {
    CGFloat width = view.frame.size.height > view.frame.size.width ? view.frame.size.width : view.frame.size.height;
    CGFloat height = view.frame.size.height < view.frame.size.width ? view.frame.size.width : view.frame.size.height;
    CGRect frame = CGRectMake(0, height - width - offset, width, width);
    DBGuestureLock *lock = [[DBGuestureLock alloc] initWithFrame:frame isUnLockType:isUnLockType];
    lock.delegate = nil;
...
//@Override
-(instancetype)initWithFrame:(CGRect)frame isUnLockType:(BOOL)isUnLockType {
    self = [super initWithFrame: frame];
    if (self) { // Draw 9 Lock Buttons
        self.isUnLockType = isUnLockType;
...
-(void)setPropertiesByState:(DBButtonState)buttonState {
    switch (buttonState) {
        case DBButtonStateNormal:
            ...
            self.lineColor = self.isUnLockType?[UIColor colorWithRed:232/255.0 green:235/255.0 blue:243/255.0 alpha:1.0]:[UIColor colorWithRed:91/255.0 green:114/255.0 blue:236/255.0 alpha:1.0];
            if (self.delegate) {
            ...
            //As default
            self.strokeColor = self.isUnLockType?[UIColor colorWithRed:232/255.0 green:235/255.0 blue:243/255.0 alpha:1.0]:[UIColor colorWithRed:91/255.0 green:114/255.0 blue:236/255.0 alpha:1.0];
            if (self.delegate) {
        ...
        case DBButtonStateIncorrect:
            ...
            //As default
            self.centerPointColor = [UIColor redColor];
            if (self.delegate) {
                if ([self.delegate respondsToSelector:@selector(colorOfButtonCircleCenterPointOnState:)]) {
            ...
            //self.circleRadius = self.circleRadius;
            [self performSelector:@selector(lockState:) withObject:[NSArray arrayWithObject:[NSNumber numberWithInteger:DBButtonStateNormal]] afterDelay:0.2f];
            break;
        default:
            break;
    }

...
- (UIImage*)icon
{
	if(nil == _icon)
	{
		_icon = [UIImage _applicationIconImageForBundleIdentifier:self.bundleIdentifier format:10 scale:3.0];
	}
...
{
    if ([_applicationProxy respondsToSelector:@selector(staticDiskUsage)]) {
...
- (BOOL)isHiddenApp
{
    if ([_applicationProxy respondsToSelector:@selector(appTags)]) {
